...
export default class Board extends React.Component {
        ...
        for (let i = 0; i < height; i++) {
            for (let j = 0; j < width; j++) {
                if (data[i][j].isMine !== true) {
                    let mine = 0;
                    const area = this.traverseBoard(i, j, data);
                    area.map(value => {
                        if (value.isMine) {
                            mine++;
                        }
    ...
    handleCellClick(x, y) {
        ...
        if (this.getVals(updatedData, "isRevealed").length === this.props.mines) {
            win = true;
            this.revealBoard();
            alert("You Win");
        }
        ...
        this.setState({
            boardData: updatedData,
            mineCount: this.props.mines - this.getVals(updatedData, "isFlagged").length,
            gameWon: win,
        });
    }
    ...
    _handleContextMenu(e, x, y) {
        ...
        if (mines === 0) {
            const mineArray = this.getVals(updatedData, "isMine");
            const FlagArray = this.getVals(updatedData, "isFlagged");
